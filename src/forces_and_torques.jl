...
function total_force_and_torque(prob::ResistanceProblem)
    forces = reshape(prob.force_vals, 3, :)
    points = prob.points
    total_force(forces, points), total_torque(forces, points)
end
...
function stresslet_tensor(prob::SwimmingProblem)
    @unpack quad_pts, nearest = prob.points
    forces = get_forces(prob)
    S_raw = 0.5*sum(forces[n] * quad_pts[:,n]' + quad_pts[:,n] * forces[n]' for n in nearest)
    S_raw - (1/3)*tr(S_raw)*I
end

function average_stresslet_tensor(prob::SwimmingProblem; period=1.0, num_ts=30)
    Ss = []
