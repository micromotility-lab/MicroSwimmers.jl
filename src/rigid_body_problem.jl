...
function orientation_ode(X,B_shear,strain_shear,Omega_shear,t)
    b1 = X[1:3]
    b2 = X[4:6]
    b3 = cross(b1,b2)
    Q = hcat(b1,b2,b3) 
    Omega_p =sum((Q*B_shear[1,:,:]*Q').*strain_shear)
    Omega_q =sum((Q*B_shear[2,:,:]*Q').*strain_shear)
    Omega_r =sum((Q*B_shear[3,:,:]*Q').*strain_shear)
    db1 = cross(Omega_shear,b1) - Omega_q*b3 + Omega_r*b2
    db2 = cross(Omega_shear,b2) + Omega_p*b3 - Omega_r*b1
...
function RigidBodyProblem(ms::MicroSwimmer; 
    ...
    B_shear = body_shape_tensor(ms; eps = 1e-5)[4:6, :, :],
    t_final=20.0,
    saveat=0.05,
    eps=0.1
    )
    ...
    rhs = (X, p, t) -> orientation_ode(X, B_shear, strain_shear, Omega_shear, t)
